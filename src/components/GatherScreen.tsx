...
import { AbsentSeatNudgePanel } from "./AbsentSeatNudgePanel";
import {
  PIECE_TYPES,
  PIECE_TYPE_META,
  ROLE_LENSES,
...
export function GatherScreen() {
  ...
  const updateFragment = useSession((s) => s.updateFragment);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <AbsentSeatNudgePanel
        participants={participants}
        contributionCount={contributionCount}
        missingContributors={missingContributors}
        startEvidenceTurn={startEvidenceTurn}
      />
          ...
          {fragments.length === 0 ? (
            ...
            <ul className="space-y-2.5">
              {fragments.map((f) => (
                ...
                >
                  {editingFragmentId === f.id ? (
                    <form
                      onSubmit={(event) => {
                        event.preventDefault();
                        if (!editTitle.trim() || !editBody.trim()) return;
                        updateFragment(f.id, { title: editTitle, body: editBody });
                    ...
                    >
                      <input
                        ...
                        aria-label={t("gather.title")}
                        className="w-full rounded-lg border border-line bg-paper px-3 py-2 text-sm font-semibold text-ink outline-none transition focus:border-accent"
                      />
                      <textarea
                        value={editBody}
                        onChange={(event) => setEditBody(event.target.value)}
                        aria-label={t("gather.body")}
                        rows={3}
                        className="w-full resize-y rounded-lg border border-line bg-paper px-3 py-2 text-sm leading-relaxed text-ink outline-none transition focus:border-accent"
                      />
                      <div className="flex justify-end gap-2">
                        <button
                          type="button"
                          onClick={() => setEditingFragmentId(null)}
                          className="rounded-md px-3 py-1.5 text-xs text-ink-soft transition hover:bg-paper-sunken"
                        >
                          {t("gather.cancel")}
                        </button>
                        <button
                          ...
                          className="rounded-md bg-ink px-3 py-1.5 text-xs font-medium text-paper transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
                        >
                          {t("gather.save")}
                        </button>
                      </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex shrink-0 gap-1 opacity-60 transition group-hover:opacity-100">
                        <button
                          onClick={() => {
                            setEditingFragmentId(f.id);
                            setEditTitle(f.title);
                            setEditBody(f.body);
                          }}
                          className="rounded-md px-2 py-1 text-[11px] text-ink-faint transition hover:bg-paper-sunken hover:text-ink"
                        >
                          {t("gather.edit")}
                        </button>
                        <button
                          ...
                          className="rounded-md px-2 py-1 text-[11px] text-ink-faint transition hover:bg-paper-sunken hover:text-tension"
                        >
                          {t("gather.remove")}
                        </button>
                      </div>
